...
st.write("#AVIZIG: Automatic Visualizations and Infographics Generator tool using Large Language Models📊")
...
if not openai_key:
    ...
    if openai_key:
        ...
        st.sidebar.write("Please enter OpenAI API key. **We do not store your API key, it is passed automatically to the code via os.getenv. Change your API Keys after usage**")
else:
    display_key = openai_key[:2] + "*" * (len(openai_key) - 5) + openai_key[-3:]
...
st.markdown(
    ...
    AVIZIG: Automatic Visualizations and Infographics Generator tool let you visualize using your dataset using LIDA and OpenAI.
    ...
    of LIDA are described in the [paper here](https://aclanthology.org/2023.acl-demo.11/).
